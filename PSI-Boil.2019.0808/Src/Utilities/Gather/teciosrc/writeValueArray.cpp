...
SPECIALIZE_MIN_MAX_ENCODING_FOR_TYPE(uint8_t) SPECIALIZE_MIN_MAX_ENCODING_FOR_TYPE(int16_t) SPECIALIZE_MIN_MAX_ENCODING_FOR_TYPE(int32_t) SPECIALIZE_MIN_MAX_ENCODING_FOR_TYPE(float) SPECIALIZE_MIN_MAX_ENCODING_FOR_TYPE(double) template <typename T, bool ___2023, int base> ___372 ___4561(FileWriterInterface& file, char const*          ___970, ___81           ___1249, size_t               ___2795, T const*             ___4297, size_t               ___4332  ) { ___372 ___2037 = ___4224; REQUIRE(file.___2039()); REQUIRE(VALID_DESCRIPTION(___970)); REQUIRE("extraID could have any value. NO_EXTRA_ID show only the description"); REQUIRE(___2795>0); REQUIRE(VALID_REF(___4297)); if ( file.___2000() ) { ASSERT_ONLY(___1391 beginningLocation = file.fileLoc()); std::string ___1416 = ___970; if ( ___1249 != ___2743 ) ___1416 += ___4185(___1249+1);
 #if defined ASCII_ANNOTATE_TYPES
___1416.append(1, ' ').append(AsciiTypeString<T>::typeString);
 #endif
if ( ___2795 == 1 ) file.fprintf("%*s  ", -___206, ___1416.c_str()); else file.fprintf("%*s\r\n", -___206, ___1416.c_str()); const int buffSize = 100; char buff[buffSize]; std::string outputBuffer; for ( size_t pos = 1; pos <= ___2795; ++pos ) { ___2037 = ___2037 && encodeAsciiValue<T, ___2023, base>(buff, buffSize, ___4297[pos-1]); outputBuffer.append(buff); if( (pos % ___4332) == 0 || (pos == ___2795 ) ) outputBuffer.append("\r\n"); else outputBuffer.append("  "); } file.fwrite(outputBuffer.c_str(), sizeof(char), outputBuffer.size()); ASSERT_ONLY(___1391 endingLocation = file.fileLoc()); ASSERT_ONLY(___1391 outputSize = (___1391)___206 + 2 + outputBuffer.size()); ___476(endingLocation - beginningLocation == outputSize); } else { file.fwrite(___4297, sizeof(T), ___2795); } ENSURE(VALID_BOOLEAN(___2037)); return ___2037; } template <typename OutType> ___372 ___4526( FileWriterInterface&        file, char const*                 ___970, ___81                  ___1249, size_t                      ___2795, ___2477 const*               ___4297, size_t                      ___4332  ) { ___2238<std::pair<OutType, OutType> > outputArray; ___372 ___2037 = outputArray.alloc(___2795); if (___2037) { for (size_t i = 0; i < ___2795; ++i) { outputArray[i].first = static_cast<OutType>(___4297[i].minValue()); outputArray[i].second = static_cast<OutType>(___4297[i].maxValue()); } ___2037 = ___4561<std::pair<OutType, OutType>, false, 0>(file, ___970, ___1249, ___2795, &outputArray[0], ___4332); } return ___2037; } template <typename T, bool ___2023> uint64_t arrayValueSizeInFile(bool ___2000) { if (___2000) return ___199<T, ___2023>::size + ASCII_SPACING_LEN; return sizeof(T); } template <typename T, bool ___2023> uint64_t arraySizeInFile(size_t ___2795, bool ___2000) { uint64_t charsPerNumber = arrayValueSizeInFile<T, ___2023>(___2000); ___476(charsPerNumber > 0); uint64_t ___3356 = static_cast<uint64_t>(___2795) * charsPerNumber; if (___2000) ___3356 += static_cast<uint64_t>(___206) + ASCII_SPACING_LEN; return ___3356; } template <typename T, bool ___2023> uint64_t valueSizeInFile(bool ___2000) { return arraySizeInFile<T, ___2023>(1, ___2000); }
 #define INSTANTIATE_FOR_TYPE(T, ___2023) \
 template ___372 ___4561< T, ___2023, 0 >( \
...
INSTANTIATE_FOR_TYPE(char, false) INSTANTIATE_FOR_TYPE(uint8_t, false) INSTANTIATE_FOR_TYPE(uint8_t, true) INSTANTIATE_FOR_TYPE(int16_t, false) INSTANTIATE_FOR_TYPE(uint16_t, false) INSTANTIATE_FOR_TYPE(uint16_t, true) INSTANTIATE_FOR_TYPE(int32_t, false) INSTANTIATE_FOR_TYPE(uint32_t, false) INSTANTIATE_FOR_TYPE(uint64_t, false) INSTANTIATE_FOR_TYPE(uint64_t, true) INSTANTIATE_FOR_TYPE(float, false) INSTANTIATE_FOR_TYPE(double, false) INSTANTIATE_FOR_TYPE(StdPairFloat, false) INSTANTIATE_FOR_TYPE(StdPairDouble, false) INSTANTIATE_FOR_TYPE(StdPairInt32, false) INSTANTIATE_FOR_TYPE(StdPairInt16, false) INSTANTIATE_FOR_TYPE(StdPairUInt8, false) template ___372 ___4561< uint32_t, false, 1 >(FileWriterInterface& file, char const* ___970, ___81 ___1249, size_t ___2795, uint32_t const* ___4297, size_t ___4332);
 #define INSTANTIATE_MIN_MAX_OUTPUT_FOR_TYPE(T) \
 template ___372 ___4526 <T>( \
 FileWriterInterface&        file, \
 char const*                 ___970, \
 ___81                  ___1249, \
 size_t                      ___2795, \
