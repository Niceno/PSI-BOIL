...
/*============================================================================*/
real PropertyMixHarm::value(const int i, const int j, const int k) const {
...
/*============================================================================*/
real PropertyMixHarm::value(const Comp & m,
                        const int i, const int j, const int k) const {
  int ox(0), oy(0), oz(0);
  if       (m==Comp::i()) {
    ox--;
  ...
  if(!bndc_a) {
    ...
    return 1.0/(0.5/value(i,j,k)+0.5/value(i+ox,j+oy,k+oz));
    ...
    if( c_disp_b != NULL ) {
     ...
     return 1.0 / (col_a/a->value(m,i,j,k)+(1.0-col_a)/b->value(m,i,j,k));
  }
}
