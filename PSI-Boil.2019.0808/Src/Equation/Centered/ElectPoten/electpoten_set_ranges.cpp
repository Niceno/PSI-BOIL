...
/******************************************************************************/
void ElectPoten::set_ranges(Vector & u, const Dir & d, const Comp & m, 
                           Range<int> * i, Range<int> * j, Range<int> * k) {

  if( d==Dir::imin() ) {i->first(u.si(m)-1); i->last(u.si(m)-1);}
  if( d==Dir::imax() ) {i->first(u.ei(m)+1); i->last(u.ei(m)+1);}

  if( d==Dir::jmin() ) {j->first(u.sj(m)-1); j->last(u.sj(m)-1);}
  if( d==Dir::jmax() ) {j->first(u.ej(m)+1); j->last(u.ej(m)+1);}

  if( d==Dir::kmin() ) {k->first(u.sk(m)-1); k->last(u.sk(m)-1);}
  if( d==Dir::kmax() ) {k->first(u.ek(m)+1); k->last(u.ek(m)+1);}
