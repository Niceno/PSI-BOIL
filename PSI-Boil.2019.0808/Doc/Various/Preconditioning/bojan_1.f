...
C==============================================================
     ...
     *              L_E(NXYZ),L_W(NXYZ),L_N(NXYZ),L_S(NXYZ),
     *              L_T(NXYZ),L_B(NXYZ),L_P(NXYZ)
...
C
      DO K=2,NKM
        DO I=2,NIM
          DO J=2,NJM
            IJK=LK(K)+LI(I)+J
            D(IJK)=SQRT( AP(IJK) - (AW(IJK)/(D(IJK-NJ) +SMALL))**2
     *                           - (AS(IJK)/(D(IJK-1)  +SMALL))**2
     *                           - (AB(IJK)/(D(IJK-NIJ)+SMALL))**2 )
          END DO
        END DO
      END DO
C
...
C
      DO K=2,NKM
        DO I=2,NIM
          DO J=2,NJM
            IJK=LK(K)+LI(I)+J
            ZK(IJK)=(RES(IJK) - AW(IJK)/(D(IJK-NJ )+SMALL)*ZK(IJK-NJ )
     *                        - AS(IJK)/(D(IJK-1  )+SMALL)*ZK(IJK-1  )
     *                        - AB(IJK)/(D(IJK-NIJ)+SMALL)*ZK(IJK-NIJ))
     *                        / (D(IJK) + SMALL)
          END DO
        END DO
      END DO
...
C
      ...
      DO K=NKM,2,-1
        DO I=NIM,2,-1
          DO J=NJM,2,-1
            IJK=LK(K)+LI(I)+J
            ZK(IJK)=(ZK(IJK) - AE(IJK)/(D(IJK+NJ )+SMALL)*ZK(IJK+NJ)
     *                       - AN(IJK)/(D(IJK+1  )+SMALL)*ZK(IJK+1)
     *                       - AT(IJK)/(D(IJK+NIJ)+SMALL)*ZK(IJK+NIJ))
     *                       / (D(IJK) + SMALL)
