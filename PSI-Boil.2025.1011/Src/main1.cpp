...
int main(int argc, char ** argv) {
...
#endif
  Scalar qsink_nucl(dom);  // qsink for nucleation
  real *** copy_planeVec;
  ...
  |  physical properties  |
  +----------------------*/
  Matter vapor(dom), liquid(dom), sapphire(dom);
  sapphire.rho    (3980.0);
  ...
  const int  nint = 1000;   // time interval for bck-output
  ...
  if( restart ) {
    ...
    tpr.bnd_update();
    tpr.exchange();
    boil::plot->plot(uvw,c,tpr,press,mdot,mu_t,qsink_nucl
                    ,"uvw-c-tpr-press-mdot-mut-qsink",999999);
#endif
  }
  ...
  for(time.start(); time.end(); time.increase()) {
    ...
    if((time.current_time()) / (tint) >= real(iint) ) {
      ...
      tpr.exchange_all();
      boil::plot->plot(uvw,c,tpr,press,mdot,mu_t,qsink_nucl,
                      "uvw-c-tpr-press-mdot-mut-qsink",iint);
