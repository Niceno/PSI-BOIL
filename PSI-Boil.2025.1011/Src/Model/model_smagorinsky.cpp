...
void Model::smagorinsky( const Momentum * mom, Scalar * mu_t, real c_s,
  ...
  for_vijk( (*mu_t), i, j, k) {
    ...
    +-----------------------------------------------------------------------*/
    real damp = 1.0;
    const real y_pl_damp = 25.0;  //25.0;
    if (dist) {
      if( ibody.on(i,j,k) ) {
        if ((*dist)[i][j][k] < *dist_max) {
          ...
          wall_function( uvw, fluid, * dist, & tau_w, & y_pl,
                         i, j, k, &tx, &ty, &tz );
          if (y_pl<=0.0) {
            boil::aout<<"y_pl is negative "<<y_pl<<"\n";
          }
          if (1.0 - exp(-pow(y_pl/y_pl_damp,3.0))<0.0) {
            boil::aout<<"1.0 - exp(-pow(y_pl/y_pl_damp),3.0) is negative "<<y_pl<<"\n";
          }
          if (y_plus) (*y_plus)[i][j][k] = y_pl;
          damp = sqrt(1.0 - exp(-pow(y_pl/y_pl_damp,3.0)));
          damp_sum += damp;
          damp_n++;
        }
      } else {
