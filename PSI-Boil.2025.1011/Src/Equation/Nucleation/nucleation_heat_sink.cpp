...
void Nucleation::heat_sink(std::vector<Site> & s, const int ns,
  ...
  if(heat_sink()) {
    ...
    if (s[ns].contain_range()) {
      ...
      for(int i=s[ns].is(); i<=s[ns].ie(); i++) {
		...
		real cps = solid()->cp(i,j,k-1);
                real p_wall_super = cps * wall_super * qsrc->dV(i,j,k-1) / time->dt(); //[W]
                /* heat sink in solid cell */
		real q_sink = (1.0-cseed) * (q_heater+p_wall_super);  //[W]
		sum_q_sink += q_sink;  //[W]
		(*qsrc)[i][j][k-1] -= q_sink;  //[W]
                if (qsink_nucl) (*qsink_nucl)[i][j][k-1] = q_sink;
                //std::cout<<"wall_super= "<<wall_super<<" Tw "<<Tw
                //         <<" Tsat "<<Tsat<<" cps= "<<cps<<" cseed "<<cseed<<"\n";
                //std::cout<<"cseed= "<<cseed<<" clr= "<<(*clr)[i][j][k]
                //         <<" vf= "<<(*vf)[i][j][k]<<"\n";
              } else {
                /* heat sink is given in one time step */
                /* unit: [kg/m3]*[m3]*[J/kg]/([m2]*[s]*[m])*[m3] = [W]  */
                real q_sink = rhov * vol_seed * latent
  ...
  s[ns].set_sink_energy(sum_q_sink*time->dt()); // [W*s]=[J]
