...
*******************************************************************************/
  ...
  // calculate color function in solid domain using given contact angle,
  // if the curvature in wall-adjacent cells is calculated with HF.
  // In case that use_HF_wall=false, curvature in wall-adjacent cells is
  // calculated with normal vector, i.e. bdcurv is called.
  if (use_HF_wall) {
    // store phi in stmp2
    for_avijk(phi,i,j,k) {
      stmp2[i][j][k] = phi[i][j][k];
    }

    update_at_walls(phi,&cangle);
    phi.bnd_update();  // necessary for symmetry BC 
    phi.exchange_all();
  }
...
#if 1
  // Young's method is good for low resolution
  norm_young(phi,true);
#else
  // mixed approach, which doesn't work for 3D parasitic current simulation. 
  for_aijk(i,j,k) {
    nx[i][j][k] = mx[i][j][k];
    ny[i][j][k] = my[i][j][k];
    nz[i][j][k] = mz[i][j][k];
...
#if 0
        if (k==sk()+4) {
          std::cout<<"curv_HF:i= "<<i<<" "<<j<<" "<<kmin<<" "<<kmax<<"\n";
        }
#endif
  ...
  if (!use_HF_wall) {
    ...
    // recover phi. phi has been modified by update_at_walls(phi,&cangle);
    for_avijk(phi,i,j,k) {
      phi[i][j][k] = stmp2[i][j][k];
    }
  }
