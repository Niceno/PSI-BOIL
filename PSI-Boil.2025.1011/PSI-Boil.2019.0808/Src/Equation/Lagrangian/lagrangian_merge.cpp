...
void Lagrangian::merge(int p) {
  ...
  for_pijk(p, i, j, k) {
    ...
    if(lagrangian_fraction(c) > 0.0) (*col)[i][j][k] = c;
    
  }
   erase(p);
   (*col).exchange_all();

  OPR("lagrangian_merge-1.cpp");
  getchar();

}

/******************************************************************************/
void Lagrangian::merge(int pa, int pb) {
  ...
  OPR("lagrangian_merge-2.cpp");
  getchar();

}

/*-----------------------------------------------------------------------------+
 '$Id: lagrangian_merge.cpp,v 1.4 2018/06/15 11:39:11 MinZhu Exp $'/ 
+-----------------------------------------------------------------------------*/
