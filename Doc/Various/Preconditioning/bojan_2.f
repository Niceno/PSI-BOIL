...
C
      DO K=2,NKM
        DO I=2,NIM
          DO J=2,NJM
            IJK=LK(K)+LI(I)+J
            D(IJK)=1.0/SQRT( AP(IJK) - (AW(IJK)*D(IJK-NJ) )**2
     *                               - (AS(IJK)*D(IJK-1)  )**2
     *                               - (AB(IJK)*D(IJK-NIJ))**2 )
          END DO
        END DO
...
C
      DO K=2,NKM
        DO I=2,NIM
          DO J=2,NJM
            IJK=LK(K)+LI(I)+J
            ZK(IJK)=(RES(IJK) - AW(IJK)*D(IJK-NJ )*ZK(IJK-NJ )
     *                        - AS(IJK)*D(IJK-1  )*ZK(IJK-1  )
     *                        - AB(IJK)*D(IJK-NIJ)*ZK(IJK-NIJ))
     *                        * D(IJK)
          END DO
        END DO
      END DO
...
C
      ...
      DO K=NKM,2,-1
        DO I=NIM,2,-1
          DO J=NJM,2,-1
            IJK=LK(K)+LI(I)+J
            ZK(IJK)=(ZK(IJK) - AE(IJK)*D(IJK+NJ )*ZK(IJK+NJ)
     *                       - AN(IJK)*D(IJK+1  )*ZK(IJK+1)
     *                       - AT(IJK)*D(IJK+NIJ)*ZK(IJK+NIJ))
     *                       * D(IJK)
