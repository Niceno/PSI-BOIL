...
*******************************************************************************/
void EnthalpyFDAdiabatic::diffusion_fd(const Scalar * diff_eddy) {
  ...
  if( !solid() ) {
    ...
    for_ijk(i,j,k){
      ...
      const real clrw = std::min(std::max(clrold[i-1][j][k],0.0),1.0);
      const real clrc = std::min(std::max(clrold[i  ][j][k],0.0),1.0);
      const real clre = std::min(std::max(clrold[i+1][j][k],0.0),1.0);
      ...
      if((clrc-0.5)*(clre-0.5)>=0){
        ...
        pp=phi[i][j][k]; //shono edit
        //pp=tsat;
        aflagp=1.0;
      }
      const real Aw = tsc * lc * vol * 2.0 / (xm*(xm+xp)) * (1.0-aflagm);
      const real Ac = tsc * lc * vol * 2.0 / (xm*xp)- tsc * lc * vol * 2.0 / (xm*(xm+xp)) * aflagm - tsc * lc * vol * 2.0 / (xp*(xm+xp)) * aflagp; //shono edit;
      const real Ae = tsc * lc * vol * 2.0 / (xp*(xm+xp)) * (1.0-aflagp);
