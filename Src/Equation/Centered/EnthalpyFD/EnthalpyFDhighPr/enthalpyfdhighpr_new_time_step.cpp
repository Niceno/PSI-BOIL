...
*******************************************************************************/
void EnthalpyFDhighPr::new_time_step(const Scalar * diff_eddy) {

  boil::timer.start("enthalpyfdhighpr new_time_step");

  if(diff_eddy == NULL){
    if(!laminar){
      boil::oout<<"### enthalpyfdhighpr_new_time_step: Error!!!\n";
  ...
  if(conv_ts.Nm1()!=1.0){
    boil::oout<<"enthalpyfdhighpr_new_time_step, forward_euler must be used!\n";
    exit(0);
  }
  if(diff_ts.Nm1()!=0.0){
    boil::oout<<"enthalpyfdhighpr_new_time_step, backward_euler must be used!\n";
    exit(0);
  }

  /* initial time step or restart */
  if(!store_clrold){
    boil::oout<<"EnthalpyFDhighPr::new_time_step()  initialize clrold"<<"\n";
    for_aijk(i,j,k){
  ...
  for_ijk(i,j,k){
    if(dom->ibody().on(i,j,k)){
      ...
      if( ((*clr)[i][j][k]-clrsurf)*(clrold[i][j][k]-clrsurf) < 0.0){
        ...
        if(num == 0){
          buff[i][j][k] = t_new;
          std::cout<<"Warning:enthalpyfdhighpr_new_time_step:num==0! proc= "
  ...
  boil::timer.stop("enthalpyfdhighpr new_time_step");
}
