...
*******************************************************************************/
  ...
  for(int mstep=1; mstep<=mmax; mstep++){
    ...
    if(errnorm<tol_ext) {
      converged = true;
      boil::oout<<"VOF::ext_vel converged after "<<mstep<<" steps, final rel. error: "<<errnorm<<"\n";
      break;
    }
  ...
  if(!converged)
    boil::oout<<"VOF::ext_vel did not converge after "<<mmax<<" steps! \n";

#if 0
  if(iext==-1)
    for_ij(i,j)
     if(fabs(eflag[i][j][2])>1.0)
       boil::oout<<i<<" "<<j<<" | "<<sca[i][j][2]<<" "<<stmp3[i][j][2]<<" | "<<mx[i][j][2]<<" "<<my[i][j][2]<<" "<<mz[i][j][2]<<boil::endl;
  exit(0);
#endif


  //boil::timer.stop("vof ext");
