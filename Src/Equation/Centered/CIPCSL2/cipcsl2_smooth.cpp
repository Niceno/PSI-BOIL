...
/******************************************************************************/
void CIPCSL2::smooth(const Scalar & sca, Scalar & scb, const int itnum) {
/***************************************************************************//**
...
*******************************************************************************/
  ...
  if (itnum>=1) {
    ...
    for(int it=0; it<itnum; it++) {
      for_ijk(i,j,k) {
        dflag[i][j][k] =0.5*scb[i][j][k] + 0.5/(1.0+6.0*c1+12.0*c2+8.0*c3)
                         *(scb[i][j][k]
    ...
    for(int it=0; it<itnum; it++) {
      for_ijk(i,j,k) {
        nmag[i][j][k] = scb[i-1][j][k] -2.0*scb[i][j][k] +scb[i+1][j][k]
      ...
      for_ijk(i,j,k){
        ...
        scb[i][j][k]=scb[i][j][k]+dtau*nmag[i][j][k];
      }
  ...
  //  boil::plot->plot(clr,scb, "clr-scb", time->current_step());

  return;
}

/*-----------------------------------------------------------------------------+
 '$Id: cipcsl2_smooth.cpp,v 1.4 2015/02/16 08:39:56 sato Exp $'/
+-----------------------------------------------------------------------------*/
