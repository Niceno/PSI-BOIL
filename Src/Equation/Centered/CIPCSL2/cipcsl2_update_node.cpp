...
*******************************************************************************/
  ...
  for(int k=1; k<=ek()+1; k++){
    ...
    scheme.f[i][j][k]=maxr(0.0,minr(1.0,tmp));
  }}}
  scheme.bdcond_f(sca);
  ...
  for(int k=1; k<=ek()+1; k++){
    ...
    scheme.sigx[i][j][k] = maxr(0.0,minr(dx,tmp));
  }}}
  scheme.bdcond_i(sca);
  ...
  for(int k=1; k<=ek()+1; k++){
    ...
    scheme.sigy[i][j][k] = maxr(0.0,minr(dy,tmp)); 
  }}}
  scheme.bdcond_j(sca);
  ...
  for(int k=0; k<=ek()+1; k++){
    ...
    scheme.sigz[i][j][k] = maxr(0.0,minr(dz,tmp));
  }}}
  scheme.bdcond_k(sca);
  ...
  for(int k=1; k<=ek()+1; k++){
    ...
    sxyz[Comp::i()][i][j][k] = maxr(0.0,minr(dz*dy,tmp));
  }}}
  ...
  for(int k=1; k<=ek()+1; k++){
    ...
    sxyz[Comp::j()][i][j][k] = maxr(0.0,minr(dz*dx,tmp));
  }}}
  ...
  for(int k=1; k<=ek()+1; k++){
    ...
    sxyz[Comp::k()][i][j][k] = maxr(0.0,minr(dx*dy,tmp));
  }}}
  for_m(m)
