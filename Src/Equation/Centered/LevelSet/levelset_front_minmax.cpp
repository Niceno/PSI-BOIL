...
*******************************************************************************/
      ...
      for (int j=phi.sj()-1; j<=phi.ej()-1; j++) {
         for (int k=phi.sk()-1; k<=phi.ek()-1; k++) {
            /* i-direction */
               phi1=phi[i  ][j][k];
               phi2=phi[i+1][j][k];
               if ( (phi1-phisurf)*(phi2-phisurf) <=0.0) {
                  frontExist=true;
                  xyz1=phi.xc(i);
                  xyz2=phi.xc(i+1);
   ...
   boil::oout <<"levelset_front_minmax:time,xmax= "<< time->current_time() 
...
real frontPosition(real xyz1, real xyz2, real phi1, real phi2){
   real xyzfront;
   if (phi1 != phi2) {
      xyzfront=xyz1+(0.0-phi1)*(xyz2-xyz1)/(phi2-phi1);
   } else {
...
/*-----------------------------------------------------------------------------+
 '$Id: levelset_front_minmax.cpp,v 1.2 2012/09/13 08:42:26 niceno Exp $'/
+-----------------------------------------------------------------------------*/
