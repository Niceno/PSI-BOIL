...
*******************************************************************************/
  ...
  for(int cc=0; cc<dom->ibody().nccells(); cc++){
    ...
    if(dom->ibody().off(m,i,j,k)) {
      ...
      // interface in east
      real clre = clr[i+1][j][k];
      if((clrc-phisurf)*(clre-phisurf)<=0.0){
        real ww = 1.0/dx_e;
        dx_e = distance_x(i,j,k,+1,t_e,clrc,clre);
        ww *= dx_e;
    ...
    if(dom->ibody().off(m,i+1,j,k)) {
      ...
      if((clrw-phisurf)*(clrc-phisurf)<=0.0){
        real ww = 1.0/dx_w;
        dx_w = distance_x(i,j,k,-1,t_w,clrc,clrw);
        ww *= dx_w;
	dtdx = grad3(ww, dx_w, dx_e, t_w, t_c, t_e, epsl);
      }
    ...
    if(dom->ibody().off(m,i,j,k)) {
      ...
      // interface in north
      real clrn = clr[i][j+1][k];
      if((clrc-phisurf)*(clrn-phisurf)<=0.0){
        real ww = 1.0/dy_n;
        dy_n = distance_y(i,j,k,+1,t_n,clrc,clrn);
        ww *= dy_n;
    ...
    if(dom->ibody().off(m,i,j+1,k)) {
      ...
      if((clrs-phisurf)*(clrc-phisurf)<=0.0){
        real ww = 1.0/dy_s;
        dy_s = distance_y(i,j,k,-1,t_s,clrc,clrs);
        ww *= dy_s;
	dtdy = grad3(ww, dy_s, dy_n, t_s, t_c, t_n, epsl);
      }
    ...
    if(dom->ibody().off(m,i,j,k)) {
      ...
      // interface in top
      real clrt = clr[i][j][k+1];
      if((clrc-phisurf)*(clrt-phisurf)<=0.0){
        real ww = 1.0/dz_t;
        dz_t = distance_z(i,j,k,+1,t_t,clrc,clrt);
        ww *= dz_t;
    ...
    if(dom->ibody().off(m,i,j,k+1)) {
      ...
      if((clrb-phisurf)*(clrc-phisurf)<=0.0){
        real ww = 1.0/dz_b;
        dz_b = distance_z(i,j,k,-1,t_b,clrc,clrb);
        ww *= dz_b;
	dtdz = grad3(ww, dz_b, dz_t, t_b, t_c, t_t, epsl);
      }
