...
+---------------------------------------------------------------------*/
  ...
  if(NX_coarse == 0) {
    boil::oout << "NX_coarse is 0..change list_diameter in dispersed.cpp"
               << '\n';
    exit(0);
  }
  if(NY_coarse == 0) { 
    boil::oout << "NY_coarse is 0.. change list_diameter in dispersed.cpp" 
               << '\n';
    exit(0);
  }
  if(NZ_coarse == 0) { 
    boil::oout << "NZ_coarse is 0.. change list_diameter in dispersed.cpp" 
               << '\n';
    exit(0);
  }
...
/*-----------------------------------------------------------------------------+
 '$Id: dispersed_cell_init.cpp,v 1.1 2015/08/19 11:29:51 badreddine Exp $'/
+-----------------------------------------------------------------------------*/
