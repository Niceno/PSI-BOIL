...
*  crude code: assume k-plane coinsides with wall
*******************************************************************************/
#ifdef DEBUG
  ...
  for (int id=0; id<id_nearRegion.size(); id++){
    ...
    if (sites[ns].active()) {
      ...
      rl = min(rl,r);
      //std::cout<<"dmicro: "<<boil::cart.iam()<<" "<<id<<" "<<rl<<"\n";
    }
  }
  ...
  if (rl==boil::exa) {
    ...
    d_return = max(slope*pow(rl,exp_slope)+b_slope, dmicro_min);
    //std::cout<<"dmicro0: "<<rl<<" "<<exp_slope<<" "<<d_return<<"\n";
  } else {
    d_return = boil::exa;
  }
