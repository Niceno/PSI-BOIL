...
class Microlayer : public Nucleation {
  public:
    ...
    void update(Scalar * hflux, Scalar * warea);
    void update(const Scalar * diff_eddy,
                        Scalar * hflux, Scalar * warea);
    void update(real & smdot_micro,const Scalar * diff_eddy = NULL);
    void update(real & smdot_micro,Scalar * hflux, Scalar * warea);
    void update(real & smdot_micro, const Scalar * diff_eddy,
                        Scalar * hflux, Scalar * warea);
    ...
    inline void set_exp_slope(real r1, real r2) { slope=r1;
           exp_slope=r2;
           boil::oout<<"Microlayer:slope is modified to "<<r1<<"*r^"<<r2<<"\n";};
