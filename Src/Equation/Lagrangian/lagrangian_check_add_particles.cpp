...
/******************************************************************************/
void Lagrangian::check_add_particles() {
/*----------------------------------------------------------------------------+
|  This function is called from the main.cpp for particle column simulation     |
|  This is to check if the particle that enters the domain are injected        |
|  inside particle already in the domain (which still are very close from the  |
|  injection points).                                                         |
|  If it turns they will be injected inside already existig particle: then     |
|  do not inject them. This doesn't happen so often, but it is crucial to     |
|  check it. Otherwise, if particle is injected in another particle, then code    |
|  crash may happen.                                                          |
+----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------+
 '$Id: lagrangian_check_add_particles.cpp,v 1.1 2018/06/15 11:39:11 MinZhu Exp $'/ 
+-----------------------------------------------------------------------------*/
