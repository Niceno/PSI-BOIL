...
real Nucleation::area_vapor(const int i
                          ...
                          , const Dir d) {
/***************************************************************************//**
*  \brief calculate area covered with vapor (phi<0.5)
*******************************************************************************/
#if 0
  boil::oout<<"area_vapor:d= "<<d<<" k "<<k<<" z "<<clr->zc(k)<<"\n";
#endif
  real area=0.0;
  real phisurf=0.5;
  ...
  real eps_clr_2D=1.0e-8;
  if(fabs(clr1-phisurf)<eps_clr_2D) {
    clr1=phisurf+copysign(1.0,clr1-phisurf)*eps_clr_2D;
  }
  if(fabs(clr2-phisurf)<eps_clr_2D) {
    clr2=phisurf+copysign(1.0,clr2-phisurf)*eps_clr_2D;
  }
  if(fabs(clr3-phisurf)<eps_clr_2D) {
    clr3=phisurf+copysign(1.0,clr3-phisurf)*eps_clr_2D;
  }
  if(fabs(clr4-phisurf)<eps_clr_2D) {
    clr4=phisurf+copysign(1.0,clr4-phisurf)*eps_clr_2D;
  }
  ...
  //if (iflag==0) {boil::oout<<"area_vapor:return area= "<<area<<"\n";}
  if (iflag==0) return area;
  if (iflag==4) return 0.0;

#if 0
  boil::oout<<"area_vapor:partially covered: "<<clr1<<" "<<clr2<<" "<<clr3<<" "<<clr4<<"\n";
#endif
