...
int main(int argc, char ** argv) {
  ...
  const int  nint = 5000;   // time interval for bck-output
  ...
  if( restart ) {
    ...
    boil::oout<<"set N_eddy: "<<sem.n_eddy()<<"\n";
#endif
  ...
  for(time.start(); time.end(); time.increase()) {
    ...
    boil::plot->plot(p,"p",1); 
    boil::plot->plot(uvw,"uvw",1); exit(0);
    ...
    //tm.tau_wall( & ns, wd, & xyz ); // tau_wall must be called after ns.discretize
    ...
    ns.discretize( &mu_t );
    tm.tau_wall( & ns, wd, & xyz );  // tau_wall access to A[]
