...
// ---------------- SEM data types ----------------
struct Eddy {
  double x;   // streamwise position (in [-sigma_rep, 0])
  double y0;  // spanwise center
  double z0;  // wall-normal center
  int8_t sgn[3]; // independent signs for {u,v,w}
};
...
// Gaussian kernel prefactor (isotropic)
constexpr double Kpref = 1.5163266492815837; // sqrt(8.0 / pow(pi,1.5))
inline double gauss_iso(double dx, double dy, double dz, double sigma){
