...
int Scalar::i(const real x) const {
  ...
  for(int i_=si(); i_<=ei(); i_++) if(x >= xn(i_) && x <= xn(i_+1)) return i_;
  return -1;
}
...
int Scalar::j(const real y) const {
  ...
  for(int j_=sj(); j_<ej()+1; j_++) if(y >= yn(j_) && y <= yn(j_+1)) return j_;
  return -1;
}
...
int Scalar::k(const real z) const {
  ...
  for(int k_=sk(); k_<ek()+1; k_++) if(z >= zn(k_) && z <= zn(k_+1)) return k_;
  return -1;
}
