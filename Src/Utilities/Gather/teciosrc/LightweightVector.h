...
inline uint64_t numBytesForNumBits(uint64_t numBits) { return (numBits + 7) / 8; } namespace { template <typename T> uint64_t const* getRealMemPtr(T* const& ___3249) { REQUIRE(VALID_REF(___3249)); uint64_t const* uint64Array = (uint64_t const*)___3249; return uint64Array-1; } } namespace { template <typename T> uint64_t* getRealMemPtr(T*& ___3249) { REQUIRE(VALID_REF(___3249)); uint64_t* uint64Array = (uint64_t*)___3249; return uint64Array-1; } } namespace tecplot { namespace ___3931 { template<typename T> class ___2238 { public:
 #if (defined _MSC_VER && __cplusplus > 199711L) || __cplusplus >= 201103L
___2238(___2238 const&) = delete; ___2238& operator=(___2238 const&) = delete; ___2238(___2238&& ___2886) : m_ptr(std::move(___2886.m_ptr))
...
m_size = std::move(___3390.m_size); ___3390.m_size = 0;
 ...
 #else
private: ___2238(___2238 const&); ___2238& operator=(___2238 const&); public:
 #endif
void ___2317( char const* container, size_t numElements) {
...
uint64_t m_size;
 #endif
inline bool allocRawData(T*& ___3249, uint64_t requestedSize) { REQUIRE(___3249 == NULL); REQUIRE(requestedSize>0); uint64_t const totalBytesRequired = sizeof(uint64_t) + requestedSize*uint64_t(sizeof(T)); uint64_t* mem = NULL; if ( sizeof(size_t) == 4 && totalBytesRequired > uint32_t(-1) ) mem = NULL; else { ___2317("LightweightVector", requestedSize); mem = (uint64_t *)malloc( static_cast<size_t>(totalBytesRequired) ); } bool ___2037 = (mem != NULL); if ( ___2037 ) { mem[0] = requestedSize; ___3249 = (T *)&mem[1]; ___2037 = true; } return ___2037; } inline void freeRawData(T*& ___3249) { REQUIRE(VALID_REF(___3249)); free(getRealMemPtr(___3249)); ___3249 = NULL; } public: ___2238() : m_ptr(NULL)
 #ifndef NO_ASSERTS
, m_size(0)
 #endif
...
m_size = 0;
 #endif
___2037 = false; } } } return ___2037; } inline bool alloc(uint64_t requestedSize, T const& padVal) { REQUIRE(empty()); REQUIRE(requestedSize>0); bool ___2037; if ( !empty() ) ___2037 = false; else { ___2037 = allocRawData(m_ptr, requestedSize); if ( ___2037 ) {
 #ifndef NO_ASSERTS
m_size = requestedSize;
 #endif
...
template<typename T> class ___3092 { public:
 #if (defined _MSC_VER && __cplusplus > 199711L) || __cplusplus >= 201103L
___3092(___3092 const&) = delete; ___3092& operator=(___3092 const&) = delete;
 #if defined _MSC_VER && _MSC_VER <= 1800 
 #if !defined NO_ASSERTS
___3092(___3092&& ___2886) : m_ptr(std::move(___2886.m_ptr)) , m_size(std::move(___2886.m_size)) {} ___3092& operator=(___3092&& ___3390) { if (this != &___3390) { m_ptr = std::move(___3390.m_ptr); m_size = std::move(___3390.m_size); } return *this; }
 #else
___3092(___3092&& ___2886) : m_ptr(std::move(___2886.m_ptr)) {} ___3092& operator=(___3092&& ___3390) { if (this != &___3390) { m_ptr = std::move(___3390.m_ptr); } return *this; }
 #endif
 #else
___3092(___3092&&) = default; ___3092& operator=(___3092&&) = default;
 #endif
 #else
private: ___3092(___3092 const&); ___3092& operator=(___3092 const&); public:
 #endif
void ___2317( char const* container, size_t numElements) {
 #ifdef LARGE_ARRAY_MEMORY_LOGGING
size_t const MEMTRACK_CUTOFF = size_t(1000)*size_t(1000); if (numElements * sizeof(T) >= MEMTRACK_CUTOFF) { FILE *file = fopen("memtrack.txt", "at"); if (file) { fprintf(file, "%s\t%" "I64u" "\t%" "I64u" "\t%s\n", container, numElements, sizeof(T), typeid(T).___2683()); fclose(file); } else throw std::bad_alloc(); }
