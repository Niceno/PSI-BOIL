...
#include "Mutex_s.h"
 #if defined _WIN32
typedef PTP_WORK pthread_t;
 #endif
struct ___2120 { ___2662 ___2492; std::vector<pthread_t> ___2646; static int ___2825(); ___2120() { ___2492 = new ___2663(); } ~___2120() { delete ___2492; } struct ThreadJobData { ___4158 m_job; ___90 m_jobData; ThreadJobData(___4158 ___2116, ___90 ___2121) : m_job(___2116) , m_jobData(___2121) {} };
 #if defined _WIN32
...
return NULL;
 ...
 #if defined _WIN32
pthread_t thr = CreateThreadpoolWork(___4160, threadJobData, NULL); SubmitThreadpoolWork(thr); ___2646.push_back(thr);
 #else
pthread_t thread; pthread_create(&thread, NULL, ___4160, (void*)threadJobData); ___2646.push_back(thread);
 #endif
unlock(); } inline void ___2120::wait() { size_t i; for(i = 0; i < ___2646.size(); ++i) { lock(); pthread_t thr = ___2646[i]; unlock();
 #if defined _WIN32
