...
public:
  ...
  void setup(real Y_min, real Y_max, real Z_min, real Z_max, real sigma_rep, real Uc, real alp);
  ...
  void accumulate(real y, real z, real s_raw[3]) const;
...
private:
  ...
  inline real gauss_iso(real dx, real dy, real dz) const {
    // Jarrin Gaussian prefactor: sqrt(8/pi^(3/2))
    const real Kpref = 1.5163266492815837;
    const real inv2s2 = 1.0 / (2.0 * sigmaRep() * sigmaRep());
    return Kpref * std::exp( - (dx*dx + dy*dy + dz*dz) * inv2s2 );
  }
  inline real tent(real dx, real dy, real dz) const {
    return  sqrt(3.0/2.0)*std::max(0.0,(1.0-std::abs(dx/sigmaRep())))
           *sqrt(3.0/2.0)*std::max(0.0,(1.0-std::abs(dy/sigmaRep())))
           *sqrt(3.0/2.0)*std::max(0.0,(1.0-std::abs(dz/sigmaRep())));
  }
