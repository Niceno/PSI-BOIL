...
/* ------------ boundary or initial conditions */
  ...
  const real tingas  = 273.15+60.0; // inlet temperature for gas phase [K]
  const real gravity = boil::g;     // [m/s2]

  /* average values of the two cases */
  const real eps0 = 0.803919; // Tint(eps=0.8039) = 273.15 + 60.0
  const real epsin  = eps0;   // inlet eps
