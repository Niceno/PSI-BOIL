/*-------------------------------------------------------------+
|            Simulation of Deen bubble column                  |
+-------------------------------------------------------------*/
...
/* boundary conditions */
 ...
 const int nholes =  7;  /* this will give 7 x 7 = 49 spargers */
...
main(int argc, char * argv[]) {
 ...
 /* this is for saving the data */
  ...
  Matter mixed(water, air, &c_sep, &c_dro, &c); 
  mixed.sigma(0.072);
  ...
  Dispersed disp (c, & c_sep, 0, uvw, time, &mixed); 
...
/* set resume to true when you want to resume the simulation */
  ...
  for(time.start(); time.end(); time.increase()) {
    ...
    if(bck_ind <= nb_bck) {
      if(time.current_time() > bck_instants[bck_ind]) {
        boil::oout << "SAVING.. " << boil::endl;
        disp.save("particles", bck_ind);
        uvw.save ("uvw",  bck_ind);
        press.save ("press",  bck_ind);
        bck_ind++;
        boil::oout<<"csint.. " << csint << boil::endl;
        boil::oout<<"cbint.. " << cbint << boil::endl;
        boil::oout<<"bck_ind.. " << bck_ind << boil::endl;
      }
    }

    /* Fetch velocity from the located point */
    real w_avg = 0.0; real l_avg = 0.0;  real g_avg = 0.0;
    if(d.contains_ijk(ixl, jyl, kzl)) {
      w_avg = 0.5 * (uvw[Comp::w()][ixl][jyl][kzl] +
                     uvw[Comp::w()][ixl][jyl][kzl+1]);
      l_avg = w_avg * c[ixl][jyl][kzl];
      g_avg = w_avg * (1.0 - c[ixl][jyl][kzl]);
    }
    boil::cart.sum_real(& w_avg);
    boil::cart.sum_real(& l_avg);
    boil::cart.sum_real(& g_avg);
    boil::oout << "timeandloc point.. " << time.current_time() <<
                   " " << w_avg << boil::endl;
    boil::oout << "LVELOCITY. " << time.current_time() <<
                   " " << l_avg << boil::endl;
    boil::oout << "GVELOCITY. " << time.current_time() <<
                   " " << g_avg << boil::endl;
...
/*-----------------------------------------------------------------------------+
 '$Id: main.cpp,v 1.1 2015/08/28 11:20:39 badreddine Exp $'/
+-----------------------------------------------------------------------------*/
