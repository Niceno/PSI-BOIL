...
const int gLevel = 2; // 1
const int NX= 60*gLevel;
const int NZ= NX*2;
...
main(int argc, char * argv[]) {
...
#if 0
  for (int i=0; i<=66; i++) {
    std::cout<<"uvw.xn "<<i<<" "<<uvw.xn_global(Comp::u(),i)<<"\n";
  }
  exit(0);
#endif
  ...
  conc.set_nredist(1);
  conc.set_itsharpen(4);  // 4
  conc.set_globalSharpen();  // necessary for ChangeGrid
#endif
  ...
  c.load("c", nrestart);
#else
  c.load("conc-phi", nrestart); // load conc-phi as c.
#endif
  uvw.exchange_all();
  p.exchange_all();
  c.exchange_all();
  conc.init();  // after ChangeGrid, init() must be called.
  boil::plot->plot(uvw,c,press, "uvw-c-press",nrestart);
  time.first_step(nrestart);
#endif
  conc.front_minmax();
  ...
  for(time.start(); time.end(); time.increase()) {
    ...
    //ns.solve(ResRat(1.0e-4)); // 2023.01.31
