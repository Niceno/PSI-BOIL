...
const real LX =   0.00016; //Hardt
const real LZ =   LX*real(NZ)/real(NX);

const real Twall=102;
const real Tsat=100;
...
main(int argc, char * argv[]) {
  ...
  //const int  ndt = 1;
  const int  nint = 500;
  const real dt  = 1.0e-4/real(Level);
  Times time(ndt, dt);
  ...
  if( input.fail() ) {
    ...
    for_avijk(tpr,i,j,k){
      if (c[i][j][k] > 0.5) {
        tpr[i][j][k] = Twall + (Tsat - Twall) * tpr.xc(i)/tpr.xc(NX/2);
      } else {
        tpr[i][j][k] = Tsat;
      }
    }
  ...
  for(time.start(); time.end(); time.increase()) {
    ...
    boil::oout<<"main:cfl_max= "<<ns.cfl_max()<<"\n";
...
/*-----------------------------------------------------------------------------+
 '$Id: main-wall-liquid-vapor-boil.cpp,v 1.1 2018/04/30 08:55:32 sato Exp $'/
+-----------------------------------------------------------------------------*/
