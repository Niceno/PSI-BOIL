/*----------------------------------------------------------------------------+
|  Rising single CO2 bubble in stagnant water                                 |
+----------------------------------------------------------------------------*/
#include "Include/psi-boil.h"
#define USE_TECMPI   // if this line is commented out, then TEC is used instead of TECMPI
...
int main(int argc, char * argv[]) {
  ...
  +---------*/
  Domain d(gx, gx, gz);
  boil::plot->plot(d,"dom",0);  // arguments for domain is same between TEC and TECMPI
  d.save("grid64.grd");
