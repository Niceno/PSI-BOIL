...
main(int argc, char * argv[]) {
  ...
  press.bc().add( BndCnd( Dir::imin(), BndType::periodic() ) );
  press.bc().add( BndCnd( Dir::imax(), BndType::periodic() ) );
  ...
  for(time.start(); time.end(); time.increase()) {
    ...
    for(int in=0; in<6; in++) {
      ...
      ns.grad(press);
      ns.convection(); // does nothing, it is for fully implicit
...
/*-----------------------------------------------------------------------------+
 '$Id: main-iterative.cpp,v 1.5 2011/05/25 11:09:23 niceno Exp $'/
+-----------------------------------------------------------------------------*/
