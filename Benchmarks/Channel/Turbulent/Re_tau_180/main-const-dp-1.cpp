...
main(int argc, char * argv[]) {
  ...
  w.bc().add( BndCnd( Dir::imin(), BndType::periodic() ) );
  w.bc().add( BndCnd( Dir::imax(), BndType::periodic() ) );
  w.bc().add( BndCnd( Dir::jmin(), BndType::dirichlet(), 0.0 ) );
  w.bc().add( BndCnd( Dir::jmax(), BndType::dirichlet(), 0.0 ) );
  w.bc().add( BndCnd( Dir::kmin(), BndType::periodic() ) );
  w.bc().add( BndCnd( Dir::kmax(), BndType::periodic() ) );
  
  Momentum ns(uvw, xyz,      time, solver, & fluid);
  Pressure pr(p,   f,   uvw, time, solver, & fluid);
  Distance di(w,   wf,  uvw, time, solver);
...
/*-----------------------------------------------------------------------------+
 '$Id: main-const-dp-1.cpp,v 1.10 2011/05/25 11:09:23 niceno Exp $'/
+-----------------------------------------------------------------------------*/
