...
main(int argc, char * argv[]) {
  ...
  |  grid(s)  |
  +----------*/
  Grid1D gx ( Range<real>(-0.2399,  1.0399), 256, Periodic::no());
  Grid1D gy ( Range<real>(-0.2399,  0.0799),  64, Periodic::no());
  Grid1D gz ( Range<real>(-0.0799,  0.0799),  32, Periodic::no());
  ...
  +-----------------------------*/
  for_m(m) {
    uvw.bc(m).add( BndCnd( Dir::imin(), BndType::inlet(), 0.78, 0.00, 0.00) );
    uvw.bc(m).add( BndCnd( Dir::imax(), BndType::outlet() ) );
    uvw.bc(m).add( BndCnd( Dir::jmin(), BndType::inlet(), 0.00, 0.76, 0.00) );
...
/*-----------------------------------------------------------------------------+
 '$Id: main-coarse.cpp,v 1.3 2011/05/25 11:09:47 niceno Exp $'/
+-----------------------------------------------------------------------------*/
