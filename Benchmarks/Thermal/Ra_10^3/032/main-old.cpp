...
#include "Centered/Enthalpy/entherature.h"
...
main(int argc, char * argv[]) {
  ...
  +----------------------*/
  Matter fluid(d);
  fluid.lambda(     sqrt(Pr/Ra) );
  fluid.mu    ( 1.0/sqrt(Pr*Ra) );

  Times time(600, 0.05); /* ndt, dt */
	
  /*----------------+
  |  linear solver  |
  +----------------*/
  Solver * solver = new CG(d, Prec::di());
  ...
  for(time.start(); time.end(); time.increase()) {
    ...
    for_vij(g,i,j)
      for(int k=g.si()+1; k<=g.ei(); k++)
         xyz[Comp::w()][i][j][k] = 0.5 * (t[i][j][k] + t[i][j][k-1]);
...
/*-----------------------------------------------------------------------------+
 '$Id: main-old.cpp,v 1.10 2011/05/25 11:09:47 niceno Exp $'/
+-----------------------------------------------------------------------------*/
